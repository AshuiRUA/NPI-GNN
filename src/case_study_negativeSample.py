...
def predict_a_case(model, loader, device, threshold):
    model.eval()
    P = 0
    N = 0
    for data in loader:
        ...
        pred_n = math.exp(model_output[0][0])
        pred_p = math.exp(model_output[0][1])
        if pred_p > threshold:
            P += 1
        else:
            N += 1
...
if __name__ == "__main__":
    ...
    log_predict_positive = open(log_path+f'/case_predict_positive.txt', 'w')
    log_predict_negative = open(log_path+f'/case_predict_negative.txt', 'w')

    threshold = float(args.threshold)

    for interaction_key in set_negativeInteractionKey_test:
        ...
        dataset_caseStudy = LncRNA_Protein_Interaction_dataset_1hop_1220_InMemory(path_dataset_caseStudy, negative_interaction_list, 1, set_interactionKey_forGenerate, set_interactionKey_cannotUse)
        case_loader = DataLoader(dataset_caseStudy, batch_size=1)
        case_result, pred_p = predict_a_case(model, case_loader, device, threshold)
        if case_result == True:
            log_predict_positive.write(f'{lncRNA_name}\t{protein_name}\t{pred_p}\n')
        else:
            log_predict_negative.write(f'{lncRNA_name}\t{protein_name}\t{pred_p}\n')
    
    log_predict_negative.close()
    log_predict_positive.close()
    time_end = time.time()
