...
def read_interaction_dataset(dataset_ptah, dataset_name):
    ...
    if dataset_name == 'NPInter2' or dataset_name == 'RPI7317':   # need_sequence_data == False
        print('开始读取xlsx文件')
        wb = load_workbook(dataset_ptah)
        ...
        for row in rows:
            ...
            [lncRNA_name, protein_name] = [col.value for col in row]
            ...
            temp_interaction = LncRNA_Protein_Interaction(temp_lncRNA, temp_protein, 1)
        ...
        print('读入的lncRNA总数：', lncRNA_count, '读入的protein总数：', protein_count, 'node总数：', lncRNA_count + protein_count)
        print('读入的interaction总数：', len(interaction_list))
    elif dataset_name == 'RPI2241' or dataset_name == 'RPI369':
        print('开始读取xlsx文件')
        wb = load_workbook(dataset_ptah)
        sheets = wb.worksheets   # 获取当前所有的sheet
        sheet = sheets[0]
...
def output_intermediate_products(dataset_name, node2vec_windowSize, interaction_list, negative_interaction_list, lncRNA_list, protein_list):
    output_path = f'data/reduce_dataset_intermediate_products/{dataset_name}'
    if not osp.exists(path=output_path):
        os.makedirs(output_path)
    output_path = f'data/reduce_dataset_intermediate_products/{dataset_name}/node2vecWindowSize={node2vec_windowSize}'
    if not osp.exists(path=output_path):
        os.makedirs(output_path)
    ...
    if not osp.exists(path=interaction_list_path):
        ...
        raise Exception('reduce_dataset_intermediate_products 已经存在')
    ...
    output_path = f'data/node2vec_result/{dataset_name}'
    if not osp.exists(path=output_path):
        os.makedirs(output_path)
    output_path = f'data/node2vec_result/{dataset_name}/node2vecWindowSize={node2vec_windowSize}'
    if not osp.exists(path=output_path):
        os.makedirs(output_path)
...
if __name__ == '__main__':
    ...
    # 正负样本读入或生成
    read_interaction_dataset(dataset_ptah=interaction_dataset_path, dataset_name=args.datasetName)  
    ...
    if args.datasetName == 'NPInter2' and args.reduce == True :
        ...
        dataset_name = f'{args.datasetName}_{args.reduceRatio}'
        ...
        # 输出缩小后的网络
        G = networkx_format_network_generation(interaction_list, negative_interaction_list, lncRNA_list, protein_list)
        graph_output_path = f'data/graph/{dataset_name}'
        output_edgelist_file(G, graph_output_path)
        # 输出中间产物
        output_intermediate_products(dataset_name, args.node2vecWindowSize, interaction_list, negative_interaction_list, lncRNA_list, protein_list)
        print('需要使用执行node2vec的到结果')
        exit()
    else:
        # 不缩小数据集
    ...
    if args.shuffle == True:
        dataset_path = f'data/dataset/_{dataset_name}_{args.hopNumber}_hop_node2vecWindowSize={args.node2vecWindowSize}_shuffled_dataset'
    else:
